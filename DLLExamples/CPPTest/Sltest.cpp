...
CSLTestWindow::CSLTestWindow()
{
    ...
    HLOCAL      HDSignalLocalMem, HDFreqRLocalMem, HDFreqILocalMem, HDCoeffLocalMem, HDResultLocalMem;      // Double type data buffer
...
afx_msg void CSLTestWindow::OnFFTSignal ()
{
    ...
    SDA_Rfft (FreqRBufferPtr, FreqIBufferPtr, CoeffBufferPtr, SIGLIB_NULL_ARRAY_INDEX_PTR, (int)Signal.GetSize (), ((SLArrayIndex_t)(SDS_Log2(FFT_LENGTH)+SIGLIB_MIN_THRESHOLD))   // Log FFT length and avoid quantization issues);            /* Perform FFT */
    SDA_Magnitude (FreqRBufferPtr, FreqIBufferPtr, SignalBufferPtr, Signal.GetSize ()); /* Calc real power fm complex */

    ::LocalUnlock (HSignalBuffer);          // Unlock the sample buffers
